<template>
    <header class="flex items-center">
        ...
        <ul class="flex items-center">
          <li>
            <nuxt-link class="text-white hover:text-teal-500 text-lg px-4 py-3"
              to="/"
              >Home</nuxt-link
            >
          </li>
          <li>
            <a class="text-white hover:text-teal-500 text-lg px-4 py-3"
              href="#"
              >Project</a
            >
          </li>
        ...
        <ul class="flex ml-auto items-center mt-2" v-else>
          <div class="flex-ml-auto" >
            <div class="dropdown inline-block relative z-10">
              <button class="bg-white text-gray-700 fotn-semibold py-4 px-6 rounded inline-flex items-center">
                <img v-if="$store.state.auth.user.image_url" :src="$axios.defaults.baseURL + $store.state.auth.user.image_url" alt="" class="h-8 rounded-full mr-2">
                <span class="mr-1">
