      ...
      <section class="container mx-auto pt-8">
        ...
        <div class="flex justify-between items-center">
          ...
          <div class="w-1/4 text-right">
            <nuxt-link
              :to="{name : 'dashboard-projects-id-edit', params: {id: campaign.data.id}}"
        ...
        <div class="flex -mx-2" v-if="campaign.data.images != null">
        ...
        <div class="block mb-2" v-if="transactions.data.length > 0">
            ...
            <div
              ...
              <div>
                ...
                <p class="text-sm text-gray-600 flex items-center mb-2">
                  Rp. {{ new Intl.NumberFormat().format(transaction.amount) }} &middot; {{ transaction.crated_at }}
                </p>
              </div>
            </div>
          </div>
        ...
        <div class="block mb-2" v-else>
          <h3>No Transactions Yet For This Campaign</h3>
        </div>
...
export default {
  async asyncData({ $axios, params }) {
    const campaign = await $axios.$get('/api/campaigns/' + params.id)
    const transactions = await $axios.$get('/api/transactions/' + params.id)
  ...
  methods: {
    ...
    async load() {
      const campaign = await this.$axios.$get('/api/campaigns/' + this.$route.params.id)
      return { campaign }
    },
    async upload(file) {
      ...
      try {
        let response = await this.$axios.post('/api/campaigns/campaign-images', formData, {
          headers: {
